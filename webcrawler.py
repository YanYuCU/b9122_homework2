...
seen = set([seed_url])    #stack of urls seen so far
...
while len(urls) > 0 and len(opened) < maxNumUrl:
    ...
    for tag in soup.find_all('a', href = True): #find tags with links
        ...
        # urljoin will check whether we should combine seed_url and childUrl
        # sometimes they don't have the same domain, then we just keep the childUrl
        # if they share the same domain, then we can combine they together
